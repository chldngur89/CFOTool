  ...
  return (
    <div className="max-w-6xl mx-auto px-5">
      <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center mb-8">
        <div className={`inline-block rounded-2xl p-6 border-2 ${isSuccess ? 'bg-green-50 border-green-200' : 'bg-slate-50 border-slate-200'}`}>
          <motion.div animate={{ rotate: [0, 10, -10, 0] }} transition={{ duration: 0.5 }} className="text-5xl mb-3">
            {isSuccess ? '🎉' : '⚔️'}
          </motion.div>
          <h2 className="text-2xl font-bold text-navy-custom mb-2">
            {isSuccess ? '승리의 전략!' : '시뮬레이션 완료'}
          </h2>
          <p className="text-slate-500 text-sm">24개월 재무 전투 시뮬레이션 결과</p>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        <motion.div initial={{ x: -50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
          <h3 className="text-[11px] font-bold uppercase tracking-widest text-slate-400 mb-6">24개월 전투 타임라인</h3>
          <div className="space-y-3 max-h-96 overflow-y-auto">
            {results.filter((_, i) => i % 2 === 0 || i === results.length - 1).map((result, index) => (
              <motion.div
                ...
                className="relative bg-slate-50 p-3 rounded-xl border border-slate-100 flex items-center justify-between"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-navy-custom font-bold text-sm">{result.month}개월차</div>
                    <div className="text-xs text-slate-400">런웨이: {result.runway.toFixed(1)}개월</div>
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <div className="text-xs text-slate-400">금고: ${(result.cash / 1000).toFixed(0)}K</div>
                </div>
                {result.month === breakEvenMonth + 1 && breakEvenMonth >= 0 && (
                  <span className="absolute right-2 top-1/2 -translate-y-1/2 bg-green-500 text-white text-[10px] px-2 py-0.5 rounded-full font-bold">손익분기</span>
                )}
                {result.month === peakCashMonth + 1 && (
                  <span className="absolute right-2 top-1/2 -translate-y-1/2 bg-amber-500 text-white text-[10px] px-2 py-0.5 rounded-full font-bold">최고점</span>
                )}
              </motion.div>
            ))}
          </div>
            ...
            {results.map((result, i) => {
              ...
              return (
                <motion.div
                  ...
                  className={`flex-1 rounded-t ${result.profit >= 0 ? 'bg-green-500' : 'bg-red-500'} border border-slate-100`}
        ...
        <motion.div initial={{ x: 50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="space-y-6">
          <div className="bg-navy-custom text-white rounded-[2rem] p-6 shadow-xl">
            <h3 className="text-xl font-bold mb-4 flex items-center gap-2"><span>📈</span> 핵심 지표</h3>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
            <h3 className="text-[11px] font-bold uppercase tracking-widest text-slate-400 mb-4">AI 전술가의 조언</h3>
            ...
            <div className="font-bold text-navy-custom text-sm mb-2">지금 실행할 액션 3가지:</div>
            <div className="space-y-3">
              {recommendations.map((rec, i) => (
                ...
                >
                  ...
                  <span className="text-slate-600 text-sm flex-1">{rec.text}</span>
                </motion.div>
              ))}
            </div>
          </div>

          <div className={`rounded-2xl p-6 border-2 text-center ${isSuccess ? 'bg-green-50 border-green-200' : finalResult.cash > 0 ? 'bg-amber-50 border-amber-200' : 'bg-red-50 border-red-200'}`}>
            <div className="text-4xl mb-3">{isSuccess ? '🏆' : finalResult.cash > 0 ? '⚠️' : '💀'}</div>
            <div className="font-bold text-xl text-navy-custom mb-2">
      ...
      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.8 }} className="flex justify-center gap-4">
